                                                                                ...
                                                                                m_pExitPoseDetector(NULL)
{
	...
	ros::init(argc, argv, "openni_user_selection");
	ros::NodeHandle nh;
    ...
    if(OpenedRecording==FALSE)
    {
        ...
        std::string configFilename = ros::package::getPath("openni_user_selection") + "/SamplesConfig.xml";
        nRetVal = m_Context.InitFromXmlFile(configFilename.c_str(), m_ScriptNode, &errors);
